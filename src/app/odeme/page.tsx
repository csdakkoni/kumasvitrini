                    ...
                    <div className="lg:col-span-2">
                        ...
                        {step === 'info' && (
                            ...
                            >
                                ...
                                <button onClick={() => setStep('shipping')} className="btn btn-primary w-full btn-lg" disabled={!formData.fullName || !formData.phone || !formData.city || !formData.district || !formData.address}>
                                    Kargo Seçimine Geç
                        ...
                        {step === 'payment' && (
                            ...
                            >
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <label
                                        className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'credit_card'
                                    ...
                                    >
                                        ...
                                        <span className="text-sm font-semibold">Kredi/Banka Kartı</span>
                                    </label>
                                    <label
                                        className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'transfer'
                                                ? 'border-primary-500 bg-primary-50 text-primary-700'
                                                : 'border-surface-200 text-surface-500 hover:border-surface-300'
                                            }`}
                                    >
                                        ...
                                        />
                                        <svg viewBox="0 0 24 24" className="w-6 h-6 mb-2" fill="currentColor">
                                        ...
                                        <span className="text-sm font-semibold">WhatsApp Sipariş</span>
                                    </label>
                                </div>
                                ...
                                <div className="border-t border-surface-100 pt-5">
                                    <p className="text-sm text-surface-500 text-center mb-3">
                                        WhatsApp üzerinden Havale/EFT siparişinizi tamamlayın.
                                    </p>
                                    <button
                                        onClick={async () => {
                                            if (isSubmitting) return;
                                            ...
                                            // 2. Open Whatsapp even if db fails (don't block the user)
                                            const waMsg = `Sipariş vermek istiyorum:\n\n${items.map(i => `• ${i.product.name} - ${i.meters}m`).join('\n')}\n\nToplam: ${formatPrice(grandTotal)}\nKargo: ${shippingOption.name}\n\nTeslimat: ${formData.fullName}, ${formData.address}, ${formData.district}/${formData.city}${result?.id ? `\n\nSipariş No: ${result.id.slice(0, 8)}` : ''}`;
                                            
                                            window.location.href = `https://wa.me/905XXXXXXXXX?text=${encodeURIComponent(waMsg)}`;
                                        }}
                                        disabled={isSubmitting}
                                        className="btn btn-whatsapp w-full btn-lg relative"
                                    >
                                        {isSubmitting ? (
                                             <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                                        ) : (
                                            ...
                                            Siparişi Tamamla & WhatsApp&apos;a Git
                                            </>
                                        )}
                                    </button>
                                </div>
